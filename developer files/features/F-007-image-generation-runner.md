# F-007 — Image Generation Runner (self-hosted batch engine)
...
> **Scope boundary.** F-007 is the **image-generation service/runner** (architecture.md §3.9 Image
...
>   time + biography) are authored by **F-009**; F-007 just executes a prompt it is handed.
> - **Guarantee identity/appearance consistency** — the reference-conditioning policy that makes it
>   "unmistakably the same girl" is **F-008** (F-007 accepts reference images and passes them to the
>   model; F-008 specifies which references and how).
> - **Plan the day's photo set** (which slots, how many angles) — that is the **Daily SFW Photo
>   Batch** (**F-010**) which *enqueues* the jobs F-007 runs.
> - **Serve media to users** — reading the archive and sending it on request is **Media Delivery**
>   (**F-011**); F-007 only *writes* the archive, never reads it for a user, never runs on the reply
>   hot path.
> - **Own intimate-content gating / adult verification** — that is **F-013** / Media Delivery; F-007
...
>   (later phases); F-007 is images only. (F-014 uses F-007 to make video *keyframes*.)
...
- **US-007-01** — As a **B2-operator running personas at scale**, I want a **reliable batch engine
...
- **US-007-02** — As an **A3 premium user**, I want photos to arrive **instantly and in premium
  quality** when I ask, so that **it feels bespoke and high-end, never like I'm waiting on a render**.
  _Narrative:_ he asks for a pic and it lands at once — because F-007 already generated it overnight;
  he never sees a "generating…" delay.

- **US-007-03** — As an **A8 skeptic user**, I want the photos to **survive close scrutiny** (zoom
...
- **US-007-04** — As the **platform operator**, I want the engine to **never leave a persona with an
...
- **US-007-05** — As a **B4 platform/engine integrator**, I want the image model to be **swappable
...
- **US-007-06** — As the **infra operator on one GPU**, I want image generation to **only run when
...
```gherkin
Feature: F-007 Image Generation Runner

  Scenario: UC-007-01 A generation job produces a stored asset
  ...
  Scenario: UC-007-02 The file and the DB row map 1:1 by MED-id
  ...
  Scenario: UC-007-03 A job carries its slot/activity/intimacy metadata onto the asset
  ...
  Scenario: UC-007-04 A failed generation is retried and never writes a half-file
  ...
  Scenario: UC-007-05 Re-running the same job is idempotent
    Given a job that already produced an asset
    When the same job key is processed again
    Then no duplicate asset is created

  Scenario: UC-007-06 The model is swappable behind the fixed job API
  ...
  Scenario: UC-007-07 Only one heavy model owns the GPU at a time
  ...
  Scenario: UC-007-08 The night batch completes the day's archive
  ...
  Scenario: UC-007-09 The runner never runs on the reply hot path
  ...
  Scenario: UC-007-10 Generation is conditioned on the persona's reference
    Given a job with the persona's reference image(s)
    When the image is generated
    Then the reference is supplied to the model as identity conditioning (F-008 owns which/how)
```
...
#### Runner & isolation
- **FR-007-01** — The image generation service must be a **self-hosted, isolated runner** with its
...
- **FR-007-02** — The runner must be **persona-agnostic**: it generates for any persona from the job
  payload (reference(s) + prompt + params + target persona), with no persona hard-coded.
- **FR-007-03** — The image **model must be swappable behind the fixed job API** — the A/B candidate
...
#### Generation
- **FR-007-04** — Given a job `{persona, reference(s), prompt, params}`, the runner must **generate
...
- **FR-007-05** — The runner must **pass the persona's reference image(s) to the model as
  conditioning** for identity consistency (the *which/how* is F-008; F-007 must accept and forward
  them).
- **FR-007-06** — Generation **parameters** (steps, guidance/CFG, resolution, seed, negative prompt)
...
- **FR-007-07** — Each finished image must be written to the external **`media/<persona_slug>/photos/`**
  library, named by its **`MEDIA_ASSET.id`** (scheme **`MED-<persona>-<nnnnn>`**), so a DB row and a
  file map **1:1** (architecture.md §5.1, §6.3).
- **FR-007-08** — For each asset the runner must insert a **`MEDIA_ASSET` row** with `persona_id`,
...
- **FR-007-09** — File writes must be **atomic** (write-temp-then-rename or equivalent): a partially
  generated/failed image must **never** appear in the archive as if it were a finished asset.
- **FR-007-10** — The runner must **only write** the archive — it must never read/serve assets for a
  user (that is Media Delivery, F-011) and never generate on the reply hot path (architecture.md
  §3.6, §3.2).

#### Job lifecycle (queue, night batch)
- **FR-007-11** — The runner must **consume generation jobs from the queue** and run them as a
  **scheduled night batch** during the persona's sleep window (architecture.md §6.1, DFD-3), never
  during awake/serving hours.
- **FR-007-12** — Job processing must be **idempotent** by a job key: re-processing the same job
  (retry, redelivery) must **not** create a duplicate asset.
- **FR-007-13** — On a **failed generation** (model error, OOM, timeout), the runner must **retry
...
- **FR-007-14** — The batch must be **resumable**: if interrupted (crash, window end), remaining jobs
  are picked up on the next run so the archive still completes.

#### GPU day/night ownership
- **FR-007-15** — The runner must **only hold the GPU when the chat LLM is unloaded** — coordinated
...
- **FR-007-16** — The runner must be **brought up and torn down cleanly** each night (start model,
  process batch, release GPU), leaving no leaked GPU memory that would starve the daytime chat model.

### Non-functional

- **NFR-007-01** — **Realism:** at the chosen model + step count, output must read as a real phone
  photo under scrutiny (believable skin, hands, background; no tell-tale AI artifacts) — the
  hyper-realism bar of `user_metrics.md` (measured by the F-007-benchmark + human acceptance).
- **NFR-007-02** — **Throughput / fits the window:** the night batch must produce a persona's full
...
- **NFR-007-03** — **Never an empty archive:** a persona must never wake with **no media** for the
...
- **NFR-007-04** — **Off the hot path:** image generation must add **zero latency** to any user
  reply; it is scheduled batch work only (architecture.md §3.2, §3.6).
- **NFR-007-05** — **Referential integrity:** every `MEDIA_ASSET` row must have its file and every
  archived file its row (1:1); no orphan rows, no orphan files (architecture.md §5.1).
- **NFR-007-06** — **Durability:** generated assets and their rows survive restarts/redeploys (the
  archive is persistent object storage / the `media/` tree, §6.2).
- **NFR-007-07** — **Isolation of environments:** the image runner's dependencies (torch/CUDA/
  diffusers/ComfyUI/LightX2V) must not conflict with the chat or video runners — separate env/image
  (architecture.md §6.2c).
- **NFR-007-08** — **Observability:** the runner must expose metrics — jobs done/failed, per-image
  latency, batch completion vs window, GPU memory, empty-archive + not-torn-down alerts
  (architecture.md §6.4).
- **NFR-007-09** — **Swappability without regression:** switching the model behind the fixed API
  must not change the job contract or the stored `MEDIA_ASSET` schema (checkable by running the same
  jobs against A and B — F-007-benchmark).
- **NFR-007-10** — **Config-driven, no redeploy:** model choice, step count, params, batch window,
