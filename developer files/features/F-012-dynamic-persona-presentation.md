# F-012 — Dynamic Persona Presentation

- **Status:** Draft
- **Summary:** The **first impression** when a user opens or selects a persona. Instead of a static
  bio card, F-012 greets with a **live, context- and time-aware welcome**: a greeting **message in her
  voice** that reflects **what she's doing right now** (her current Life Engine slot — F-006) and
  **the time of day**, paired with a **fresh, fitting photo** pulled from today's archive (F-010) via
  the delivery path (F-011 selection). Opening her at 8am shows a morning her ("just woke up ☀️" + a
...
> **Scope boundary.** F-012 owns the **persona-selection presentation moment**: composing the
...
>   order are **F-001** (architecture.md §1); F-012 provides the *content* shown once a persona is
>   picked.
> - **Generate images** — the photo comes from the F-010 archive via F-011-style selection; no hot-path
>   generation.
> - **Own the ongoing conversation** — after the greeting, normal chat is F-002/F-003; F-012 is only
>   the opening beat.
...
- **US-012-01** — As a **first-time visitor**, when I open a persona, I want her to feel **alive and
...
- **US-012-02** — As an **A1/A2 emotionally-driven user**, I want the greeting to **match the time of
...
- **US-012-03** — As a **returning user**, I want the presentation to **feel different each time**, so
...
- **US-012-04** — As an **A8 skeptic user**, I want the welcome photo to be **consistent with her and
...
- **US-012-05** — As a **B1 creator**, I want each persona's welcome to **express her character**, so
...
flowchart TD
    ...
    C --> D["Select a fitting fresh photo from today's archive (F-010 via F-011 selection)"]
...
```gherkin
Feature: F-012 Dynamic Persona Presentation

  Scenario: UC-012-01 Greeting reflects her current activity and time
  ...
  Scenario: UC-012-04 Welcome photo is consistent and coherent
    Given the greeting mentions a place
    When the photo is chosen
    Then it is the same girl (F-008) and matches the mentioned moment (F-009/F-010 tags)

  Scenario: UC-012-05 Single combined message (no double nudge)
  ...
  Scenario: UC-012-06 Welcome photo is always SFW
    Given any persona
    When the welcome photo is chosen
    Then it is SFW regardless of the persona

  Scenario: UC-012-07 No hot-path generation for the welcome
  ...
  Scenario: UC-012-08 Empty archive degrades gracefully
...
- **FR-012-01** — On persona selection, F-012 must compose a **greeting message in her voice** that
  reflects her **current Life Engine slot** (F-006) and the **local time of day**.
- **FR-012-02** — The greeting must be paired with a **fresh, fitting photo** selected from today's
  archive (F-010) using F-011-style context matching (time/activity tags).
- **FR-012-03** — The greeting + photo must be delivered as **one combined message** with any keyboard/
  markup attached — **never** an intro line followed by a separate "say something" prompt (project
  feedback 2026-07-12).
- **FR-012-04** — The presentation must **vary across opens** (different time/activity → different
  greeting + photo); it must not be a fixed promo card.
- **FR-012-05** — The welcome photo must be **identity-consistent** (F-008) and **coherent** with the
  greeting's narrated moment (F-009/F-010 tags).
- **FR-012-06** — The welcome photo must always be **SFW**, regardless of persona or user.
- **FR-012-07** — The welcome must **never generate on the hot path** — the photo is selected from the
  pre-built archive (ties F-007 NFR-007-02 / F-011 FR-011-04).
- **FR-012-08** — Presentation must **degrade gracefully** when no archive photo exists yet — a
  config-default greeting (optionally text-only), never an error or a broken image.
- **FR-012-09** — The greeting content/tone must **honor per-persona character** (configurable voice/
  style), so the roster feels like distinct people from the first tap.
- **FR-012-10** — F-012 provides only the **content** of the selection moment; gallery rendering,
...
- **FR-012-11** — After the greeting, control must **hand off to normal chat** (F-002/F-003) — F-012
  does not manage the ongoing conversation.

### Non-functional

- **NFR-012-01** — **Instant (CRITICAL):** the greeting card is composed and sent with no generation
  latency — it is a lookup + text compose (ties F-011 NFR-011-01).
- **NFR-012-02** — **Freshness/variety:** across opens the card visibly varies (time/activity/photo),
  measurable over repeated opens.
- **NFR-012-03** — **Coherence:** greeting text and photo agree (same moment), human-judged on a
  sample.
- **NFR-012-04** — **Identity:** the welcome photo is unmistakably her (F-008), human/metric-judged.
- **NFR-012-05** — **Single-message UX:** exactly one outbound message for the greeting (no double
  nudge) — assertable.
- **NFR-012-06** — **Graceful fallback:** archive-empty path never errors or shows a broken image.
- **NFR-012-07** — **Config-driven:** greeting style/tone per persona tunable without code change.
- **NFR-012-08** — **Safety:** welcome photo is always SFW; never an intimate asset at the entry
  moment.

---

## 5. Coverage note
Tested in `developer files/tests/F-012-dynamic-persona-presentation.md`: time/activity-aware greeting
