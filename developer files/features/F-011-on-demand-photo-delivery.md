# F-011 — On-Demand Photo Delivery

- **Status:** Draft
- **Summary:** Serves a photo **into the live chat** — the consumer side of the day archive. When the
  user asks ("send a pic", "what are you up to?") or when she **proactively** shares one, F-011 picks
  the **right already-generated** photo from today's archive (F-010) — matched to **the current moment**
  (time-of-day, what she just said she's doing via F-006, her mood) — **marks it as sent so it is never
  repeated** to that user, attaches a short caption **in her voice** (F-002/F-003), and delivers it via
  the Media Delivery path (architecture.md §3.6). It **never generates on the reply hot path** — it only
  selects and sends a pre-made asset, so photo delivery is instant. It also respects **relationship
  gating** (F-005): how freely/often she shares, and whether a request is in-bounds, depends on the
  bond stage. This is the "**she sends me a real, fitting photo right now**" moment
  (`user_metrics.md` engagement / believability).

> **Scope boundary.** F-011 owns **selecting and sending an SFW photo into an ongoing chat**, tracking
> per-user sent history, and pacing/gating by relationship. It does **not**:
> - **Generate images** — it only picks from the F-010 archive (built by F-007/F-008/F-009). If nothing
>   fits, it degrades (see FR); it does not render on demand.
> - **Fill the archive** — that's **F-010**.
> - **Do the first-selection greeting card** — the persona-selection welcome photo+message is **F-012**
>   (a distinct entry moment); F-011 is mid-conversation delivery.
> - **Deliver intimate content** — intimate photo requests/gating/delivery are **F-013**; F-011 is SFW
>   delivery (it hands intimate requests to F-013's gate).
> - **Own the relationship model** — stage/affinity come from **F-005**; F-011 only *reads* them to pace
>   sharing.
> - **Write her conversational text** — the caption voice is F-002/F-003; F-011 requests a caption, it
>   doesn't author persona voice.

---

## 1. User stories

- **US-011-01** — As an **A3/A8 user**, when I ask her for a photo, I want her to **actually send a
  fitting one right away**, so that **she feels like a real person with a camera roll, not a bot that
  dodges**.
  _Narrative:_ "send a selfie?" → seconds later a photo that matches what she said she's doing, with a
  playful caption.

- **US-011-02** — As an **A1/A2 user**, I want her to **sometimes share a photo on her own** when it
  fits the conversation, so that **it feels like she's living her day and thought of me**.
  _Narrative:_ mid-chat she drops "look at this sunset 🌅" with a photo, unprompted.

- **US-011-03** — As an **A8 skeptic user**, I want her to **never resend the same photo**, so that
  **I can't catch her recycling a stock image**.
  _Narrative:_ every photo he's received is unique; nothing repeats.

- **US-011-04** — As an **A3 premium user**, I want the photo to **match the time and what she's
  doing**, so that **her pictures and her words always agree**.
  _Narrative:_ at 11pm the photo is an evening-at-home shot, not a bright noon gym pic.

- **US-011-05** — As the **platform operator**, I want photo delivery to be **instant (no hot-path
  generation)** and **paced by the relationship**, so that **latency stays low and sharing feels
  earned, not spammy**.
  _Narrative:_ a brand-new user isn't flooded with intimate-adjacent selfies; a bonded user gets more
  freely — all served from the pre-built archive.

---

## 2. User flows

### On request
```mermaid
flowchart TD
    A["User: 'send a pic'"] --> B["Classify: SFW photo request (intimate → hand to F-013 gate)"]
    B --> C["Read current context: time-of-day, current activity (F-006), mood, relationship stage (F-005)"]
    C --> D["Select best-matching unsent asset from today's archive (F-010)"]
    D --> E{"Match found?"}
    E -- yes --> F["Mark sent (per-user), request caption in her voice, deliver (§3.6)"]
    E -- no --> G["Degrade: nearest-fit or in-voice 'can't right now', never generate on hot path"]
```

### Proactive share (fits the moment)
```mermaid
flowchart TD
    A["Conversation touches her current activity"] --> B["Pacing allows a proactive share (relationship + recency)"]
    B --> C["Pick a fitting unsent asset → caption → send"]
```

---

## 3. Use cases (Gherkin)

```gherkin
Feature: F-011 On-Demand Photo Delivery

  Scenario: UC-011-01 Deliver a fitting photo on request
    Given today's archive has unsent shots for the current slot
    When the user asks for a photo
    Then a matching unsent asset is selected, marked sent, captioned in her voice, and delivered

  Scenario: UC-011-02 Never repeat a photo to the same user
    Given a user has already received asset X
    When another photo is requested
    Then asset X is excluded from selection

  Scenario: UC-011-03 Selection matches current time/activity
    Given it is late evening and she's home
    When a photo is selected
    Then an evening/home-tagged asset is chosen over a morning/gym one

  Scenario: UC-011-04 Delivery never generates on the hot path
    Given a photo request
    When it is served
    Then only a pre-built asset is selected; no image generation occurs inline

  Scenario: UC-011-05 Relationship gates sharing pace
    Given a brand-new relationship
    When photos are requested rapidly
    Then sharing is paced per the bond stage (F-005), not unlimited

  Scenario: UC-011-06 Intimate request is handed to the gate
    Given the user asks for an intimate photo
    When classified
    Then it is routed to F-013's gating, not served from the SFW archive

  Scenario: UC-011-07 No fitting asset degrades in voice
    Given no unsent asset fits and the archive is exhausted
    When a photo is requested
    Then she responds in-voice (nearest-fit or a natural deflection), never an error or a repeat

  Scenario: UC-011-08 Proactive share fits the moment
    Given the conversation matches her current activity and pacing allows
    When she shares proactively
    Then a fitting unsent asset is sent with an in-voice caption
```

---

## 4. Requirements

### Functional

- **FR-011-01** — On an SFW photo request, F-011 must **select an asset from today's archive** (F-010)
  matched to the **current context** (time-of-day, current activity/location from F-006, mood).
- **FR-011-02** — F-011 must **track per-user sent history** and **never resend** an asset the user has
  already received (no repeats).
- **FR-011-03** — Selection must **prefer the closest slot match** (time-of-day/activity/location tags
  in `meta_json`) and fall back sensibly when an exact match is unavailable.
- **FR-011-04** — Delivery must **never generate on the reply hot path** — only pre-built assets are
  served (ties F-007 NFR-007-02); generation is exclusively the night batch.
- **FR-011-05** — Each delivered photo must carry a **short caption in her voice** (requested from
  F-002/F-003), not a bare image.
- **FR-011-06** — Sharing must be **paced/gated by the relationship stage** (F-005) — request
  in-bounds check and frequency limits depend on the bond; a new user is not flooded.
- **FR-011-07** — **Intimate requests** must be **classified and routed to F-013's gate**, not served
  from the SFW archive.
- **FR-011-08** — When **no fitting/unsent asset** exists, F-011 must **degrade in-voice** (nearest-fit
  or a natural deflection), never surface an error, a placeholder, or a repeat.
- **FR-011-09** — F-011 must support **proactive sharing**: when the conversation matches her current
  activity and pacing allows, she may send a fitting unsent asset unprompted.
- **FR-011-10** — Delivery must go through the **Media Delivery path** (architecture.md §3.6) and record
  the send (which user, which asset, when) for history and audit.
- **FR-011-11** — Selection/pacing/caption behavior must be **config-driven** (match weighting,
  per-stage frequency caps) without code changes.

### Non-functional

- **NFR-011-01** — **Instant delivery (CRITICAL):** photo delivery adds no generation latency — it is a
  DB/asset lookup + send; p95 well under the chat reply budget (ties F-007 NFR-007-02).
- **NFR-011-02** — **No repeats (CRITICAL):** provably no asset is sent twice to the same user.
- **NFR-011-03** — **Context fit:** on a labeled sample, the delivered photo matches the current
  time/activity at a high rate (human-judged).
- **NFR-011-04** — **Pacing correctness:** per-stage frequency caps are honored (a new user cannot
  extract unlimited photos).
- **NFR-011-05** — **Graceful exhaustion:** an exhausted archive degrades in-voice, never errors.
- **NFR-011-06** — **Per-user isolation:** one user's sent-history/pacing never affects another's
  selection.
- **NFR-011-07** — **Config-driven:** weighting and caps tunable without code change.
- **NFR-011-08** — **Safety:** SFW path never serves intimate assets; the classifier defaults to the
  SFW/gate-routed side on ambiguity.

---

## 5. Coverage note
Tested in `developer files/tests/F-011-on-demand-photo-delivery.md`: context-matched selection,
per-user no-repeat, slot-preference fallback, hot-path-free delivery, in-voice caption request,
relationship pacing/gating, intimate routing, graceful exhaustion, proactive-share pacing, send
recording, and config-driven weighting are all automatable with fakes; **real context-fit quality**
is human-judged (marked). 5 US / 8 UC / 11 FR / 8 NFR.
