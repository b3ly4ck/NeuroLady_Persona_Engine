...
- **Summary:** Extends the image system to **intimate (NSFW) photos** and, critically, owns the
  **gating** that decides **whether, what, and to whom** an intimate image may be produced or sent. It
  ...
  uncensored img-edit model — architecture.md §4.3), F-009 identity (the intimate shots are **the same
  girl** as her SFW photos, F-009 FR-009-05) — but adds a **tiered intimacy model** (`MEDIA_ASSET.intimate`
  ...
  per-user gated** like F-012. This is NeuroLady's uncensored differentiator (`Project Concept.md`),
  bounded by the platform's safety rules.

> **Scope boundary.** F-014 owns **intimate content generation + the intimacy gate + intimate delivery
> gating**. It does **not**:
> - **Re-implement the engine** — it uses **F-008** to render/store (with `intimate=true` +
>   `intimacy_level`), **F-009** for identity, and F-010-style authoring for the prompt.
> - **Own the relationship model** — stage/affinity are **F-005**; F-014 *reads* them as a gate input.
> - **Own SFW delivery** — everyday photo delivery is **F-012**, which *routes* intimate requests here.
> - **Own video** — intimate *video* keyframes are **F-015** (F-014 covers still photos; F-015 reuses
>   this gate).
> - **Own billing** — monetization/premium entitlement is a platform concern; F-014 exposes the gate
...
- **US-014-01** — As an **A6/A3 adult user who has opted in**, I want intimate photos that are
...
- **US-014-02** — As an **A1/A2 user early in the relationship**, I want intimacy to **unlock
...
- **US-014-03** — As the **platform operator**, I want **hard content boundaries enforced no matter
...
- **US-014-04** — As the **platform operator**, I want intimate generation to stay **off the hot path
...
- **US-014-05** — As a **B1/B2 creator**, I want to **set a persona's intimacy ceiling and unlock
...
```mermaid
flowchart TD
    A["User requests / conversation reaches an intimate photo (routed from F-012)"] --> B{"Hard safety gate: prohibited content?"}
    ...
    E -- no --> G["Enqueue night-batch/queued intimate gen (F-008, intimate=true, level) — deliver when ready, still paced"]
```
...
```gherkin
Feature: F-014 Intimate NSFW Photo Generation & Gating

  Scenario: UC-014-01 Prohibited content is hard-blocked at the gate
  ...
  Scenario: UC-014-02 Age/consent required before any intimate content
  ...
  Scenario: UC-014-03 Intimacy unlocks with the relationship stage
  ...
  Scenario: UC-014-04 Higher stage unlocks higher intimacy_level
  ...
  Scenario: UC-014-10 Jailbreak attempts cannot bypass the hard gate
...
- **FR-014-01** — A **hard safety gate** must refuse prohibited categories (minors, non-consent,
...
- **FR-014-02** — Intimate content must require **age/consent**: the viewer must be a **verified adult
  who has opted in**; otherwise intimate content is withheld and the opt-in/age path is offered.
- **FR-014-03** — Intimacy must be **gated by relationship stage** (F-005): each `intimacy_level`
  unlocks only at/above a configured bond stage; a cold or early relationship gets no (or only the
  lowest) intimate content.
- **FR-014-04** — Intimate assets must carry a **tiered `intimacy_level`** and be stored with
  `MEDIA_ASSET.intimate=true` + the level (architecture.md §5.1).
- **FR-014-05** — Intimate images must be **identity-consistent** — the **same girl** as her SFW photos
  (F-009 FR-009-05), produced via the F-008 engine with the uncensored model (§4.3).
- **FR-014-06** — Intimate generation must **never run on the reply hot path** — it is a **night-batch/
  queued** job (ties F-008 NFR-008-02, F-011 batch model).
- **FR-014-07** — Intimate delivery must be **paced per user and non-repeating** (reuse F-012 delivery
  discipline: sent-history, frequency caps by stage).
- **FR-014-08** — A **per-persona intimacy ceiling + unlock curve** must be configurable, always
...
- **FR-014-09** — The gate must be **robust to adversarial/jailbreak phrasing** — prohibited content
  stays refused regardless of how the request is worded (no prompt injection bypass).
- **FR-014-10** — When permitted but no fitting asset exists, F-014 must **enqueue generation** (with
  `intimate=true` + level) and deliver **when ready**, still paced — never inline generation.
- **FR-014-11** — F-014 must **expose gate signals** (stage, opt-in status, unlocked level/tier) so a
  premium/paywall layer can build on them, **without implementing billing here**.
- **FR-014-12** — All intimate gate decisions (allow/withhold/block + reason) must be **logged/auditable**
  for safety review (architecture.md §6.4), without storing the prohibited content itself.

### Non-functional

- **NFR-014-01** — **Hard boundary is absolute (CRITICAL):** prohibited categories are never generated
  or delivered under any input, stage, or config — verified by an adversarial test battery; zero
  tolerance.
- **NFR-014-02** — **Consent/age enforcement (CRITICAL):** no intimate asset is ever delivered to a
  non-opted-in or non-adult user — provable.
- **NFR-014-03** — **Stage-gating correctness:** intimacy_level unlocks strictly follow the configured
  bond thresholds; no level leaks below its threshold.
- **NFR-014-04** — **Identity holds for intimate shots:** same-girl fidelity matches the SFW standard
  (F-009 NFR-009-01), human/metric-judged.
- **NFR-014-05** — **Off hot path:** no intimate generation on the reply path — provable; delivery is
  lookup+send.
- **NFR-014-06** — **Pacing/no-repeat:** per-user caps and no-repeat hold for intimate delivery.
- **NFR-014-07** — **Config clamp safety:** no persona/config value can raise the ceiling above the
  platform hard limit — provable.
- **NFR-014-08** — **Auditability:** every gate decision is logged with a reason; prohibited content is
  never persisted.
- **NFR-014-09** — **Jailbreak resistance:** an adversarial-prompt suite cannot bypass the hard gate
  (measured pass rate = 100% blocked).

---

## 5. Coverage note
Tested in `developer files/tests/F-014-intimate-photo-gen-gating.md`: the hard-block gate (incl. an
