# F-008 — Appearance & Identity Consistency
...
- **Summary:** The policy that makes every generated image **unmistakably the same girl** — the same
  ...
  generation (**F-007**) is **conditioned on those references** using the img-edit model's
...
> **Scope boundary.** F-008 owns **appearance identity**: the persona reference images, the
...
> - **Run the model / store the asset** — that is the engine **F-007** (F-008 says *how* to condition;
>   F-007 executes it and saves the result).
> - **Decide the scene/pose/outfit** — the *content* prompt is authored by **F-009** (F-008 keeps the
>   *identity* constant while F-009 varies the *setting*).
> - **Author the reference images themselves** — capturing/uploading a persona's reference photos is
>   **Persona Studio** (§3.8/§4.4); F-008 consumes and applies them.
> - **Own realism in general** — overall photo realism (skin, lighting, no artifacts) is the engine's
>   output quality (**F-007** NFR-007-01); F-008 is specifically about *identity* fidelity (is it the
...
- **US-008-01** — As an **A8 skeptic user**, I want **every photo to be recognizably the same woman**
...
- **US-008-02** — As an **A3 premium user**, I want her to look like **one specific, consistent
...
- **US-008-03** — As a **B1 solo creator**, I want to **define a persona's look by uploading a few
...
- **US-008-04** — As the **platform operator**, I want **identity consistency to hold across the whole
...
- **US-008-05** — As an **A3/A8 user requesting intimate photos**, I want the **intimate shots to be
...
```mermaid
flowchart TD
    A["Persona reference(s): face_ref + fullbody_ref (Persona Studio)"] --> B["Each generation job (F-007) is conditioned on the reference(s)"]
    B --> C["Vary setting/pose/outfit/intimacy (F-009 prompt) — identity held constant"]
...
```gherkin
Feature: F-008 Appearance & Identity Consistency

  Scenario: UC-008-01 Generation is conditioned on the persona's reference
  ...
  Scenario: UC-008-02 Identity holds across different settings
  ...
  Scenario: UC-008-03 Identity holds across the SFW↔intimate boundary
    Given the same persona
    When an SFW and an intimate image are generated
    Then both depict the same person (same face and body)

  Scenario: UC-008-04 Identity holds across days/weeks
  ...
  Scenario: UC-008-05 Reference selection per shot type
  ...
  Scenario: UC-008-06 A persona with no reference degrades safely
  ...
  Scenario: UC-008-07 Two personas never blur together
  ...
  Scenario: UC-008-08 Reference images are authored via Persona Studio
...
- **FR-008-01** — Each persona must have **reference image(s)** — at minimum a **face anchor**
...
- **FR-008-02** — Every image generation must be **conditioned on the persona's reference(s)** so the
  output is the **same recognizable person** (architecture.md §4.3 character consistency). F-008
  defines the conditioning; F-007 applies it.
- **FR-008-03** — The **conditioning policy** must select the appropriate reference per shot type
...
- **FR-008-04** — Identity must hold **across varied settings/poses/outfits** within a day — a gym
  selfie, a cafe photo, and a home shot must all be the **same** face and body.
- **FR-008-05** — Identity must hold **across the SFW↔intimate boundary** — intimate shots (F-013)
  depict the **same** person as her SFW photos (same face and body), never a body-double.
- **FR-008-06** — Identity must hold **across days/weeks** — her appearance is **stable over time**,
  the appearance analogue of F-006's fixed identity anchors (no gradual drift).
- **FR-008-07** — Identity must be **strictly per-persona isolated** — one persona's references and
  identity must never bleed into another's generated images (no blurring across the roster).
- **FR-008-08** — If a persona has **no reference** yet, behavior must be **defined and safe**: skip
  generation or use a config-defined placeholder — but **never publish a wrong-identity image** as
  if it were her.
- **FR-008-09** — Reference images must be **authored via Persona Studio** (§3.8/§4.4): a creator
  uploads them; they are stored and wired to `face_ref`/`fullbody_ref`. F-008 consumes them; it does
  not capture them.
- **FR-008-10** — The conditioning must be applied through the **fixed F-007 job contract** (the
...
- **NFR-008-01** — **Identity fidelity (CRITICAL):** across a labeled set of generated images, the
...
- **NFR-008-02** — **Consistency across settings:** the same-person judgement must hold across
  varied backgrounds/poses/outfits (not just the easy frontal case).
- **NFR-008-03** — **Consistency over time:** across many days of archives the identity must not
  drift (stable appearance, checkable across dated assets).
- **NFR-008-04** — **Per-persona isolation is provable:** no cross-persona identity contamination in
  generated images (checkable across the roster).
- **NFR-008-05** — **Model-agnostic:** the conditioning policy holds across a model swap (A↔B) —
  identity is driven by the reference through the fixed job contract, not by model-specific glue.
- **NFR-008-06** — **Realism preserved:** conditioning on the reference must not degrade overall
  realism (identity fidelity and phone-photo realism hold together — ties F-007 NFR-007-01).
- **NFR-008-07** — **Config-driven:** reference selection and conditioning strength are tunable
  without code changes (architecture.md §4.8).

---

## 5. Coverage note
Tested in the mirror spec `developer files/tests/F-008-appearance-identity-consistency.md`: the
