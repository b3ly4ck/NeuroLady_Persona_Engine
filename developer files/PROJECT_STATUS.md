...
- **F-005 + F-006 integrated into `master` — the full F-001…F-006 stack is now on one branch, green.**
  ...
  features coexist and are both wired into the live turn:
  - **`models.py`** — kept **all** new entities side by side: `Relationship` + `RelationshipReflection`
    (F-005) **and** `DailyPlan`/`Reflection`/`BiographyLayer`/`Goal` (+`GoalStatus`, F-006), each with
    its own `created_at`/`updated_at` (the auto-merge had tangled the shared timestamp block between
    `Relationship` and `Goal`).
  - **`orchestrator.py`** — `handle_turn` now fuses **both** context blocks into the single system
    message: the F-005 stage-gated behaviour directive + private summary **and** the F-006 current-
    activity line; both `_relationship_block` and `_life_engine_block` kept; module docstring updated
    to name F-004/F-005/F-006. Imports had already auto-merged (both `rel_store` and `life_store`).
  - **`prompts/__init__.py`** — add/add conflict resolved (one loader, example names both assets).
  - **Result:** full suite **357 passed, 33 skipped** (169 base F-001…F-004 + 101 F-005 + 87 F-006;
    the 33 skips are the by-design manual real-device `TC-US-*` + live-model/statistical/load-harness
    tests, same category the project has always run out-of-band). VERSION → `0.30.0`.
...
- **Removed the Welcome/Start screen (S1) — `/start` now opens Choose Lady directly (docs-first,
  ...
  the gallery; the user asked to drop it. Docs updated first, then code:
  ...
  - **Code** — `cmd_start` no longer branches on new-vs-returning; it always renders the gallery.
    ...
    merged/retired, net −1). Live-verified by the user ("работает +- верно"). This fix was then
    merged forward into `feature/f-003-human-likeness` so its live testing reflects the current
    onboarding flow.
